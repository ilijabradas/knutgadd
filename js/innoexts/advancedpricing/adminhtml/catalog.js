...
var GroupPriceGridControl = Class.create(GridControl, {
    ...
    getStoreElement : function(trEl) {
        return this.getColumnElementByName(trEl, 'store_id');
    }, 
    
    onChangeWebsite : function(trEl) {
        var websiteEl   = this.getWebsiteElement(trEl);
        if (websiteEl) {
            var websiteId   = websiteEl.getValue();
            var storeEl     = this.getStoreElement(trEl);
            if (storeEl) {
                var optionWebsiteId     = null;
                var optionStoreId       = null;
                var storeId             = storeEl.getValue();
                storeEl.select('optgroup,option').each(function(optionEl) {
                    optionWebsiteId = $(optionEl).readAttribute('website:id');
                    ...
                    if (String(optionEl.tagName).toLowerCase() === 'option') {
                        optionStoreId = $(optionEl).readAttribute('store:id');
