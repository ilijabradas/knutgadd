...
/* */
    ...
    public function getCountryHtmlSelectChina($type)
    {
        $customerIp = isset($_SERVER["HTTP_CF_CONNECTING_IP"]) ? $_SERVER["HTTP_CF_CONNECTING_IP"] : $this->getAddress()->getCountryId();
        $countryId = (($storeId = Mage::app()->getStore()->getCode()) == 'cn') ? 'CN' : Mage::getModel('geoip/country')->getCountryByIp($customerIp);
        // echo ($customerIp . ' ' . $countryId);
        if (is_null($countryId)) {
            $countryId = Mage::helper('core')->getDefaultCountry();
        }
        $select = $this->getLayout()->createBlock('core/html_select')
