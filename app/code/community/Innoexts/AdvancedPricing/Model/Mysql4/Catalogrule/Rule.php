 ...
 */
class Innoexts_AdvancedPricing_Model_Mysql4_Catalogrule_Rule 
    extends Mage_CatalogRule_Model_Mysql4_Rule 
{
     ...
     *
     * @return Innoexts_AdvancedPricing_Model_Mysql4_Catalogrule_Rule
     */
    ...
    )
    {
        ...
        if ($productFlatHelper->isEnabled() && $productFlatHelper->isBuiltAllStores()) {
            foreach ($this->_app->getStores(false) as $store) {
                ...
                foreach ($helper->getCoreHelper()->getStoreIdsByWebsiteId($websiteId) as $storeId) {
            ...
            foreach ($this->_app->getStores(false) as $store) {
                ...
                foreach ($helper->getCoreHelper()->getStoreIdsByWebsiteId($websiteId) as $storeId) {
                    ...
                    foreach ($currencies as $currency) {
                        $discountAmount         = $catalogRuleHelper->getCompoundDiscountAmount($rule, $storeId, $currency);
                        $subDiscountAmount      = $catalogRuleHelper->getCompoundSubDiscountAmount($rule, $storeId, $currency);
                        $selectByStore          = $rule->getProductFlatSelect($storeId)
        ...
        if (!$this->getVersionHelper()->isGe1800()) {
            ...
            if (!$this->getVersionHelper()->isGe1600()) {
                $queryStart = 'INSERT INTO '.$this->getTable('catalogrule/rule_product').' (
                    rule_id, from_time, to_time, website_id, store_id, currency, customer_group_id, product_id, action_operator,
            ...
            try {
                $this->insertRuleData2($rule, $websiteIds, $storeIds, $currencies);
                $write->commit();
            } catch (Exception $e) {
                $write->rollback();
                throw $e;
    ...
    protected function addWebsitePriceJoin($select)
    {
        ...
        $select->joinLeft(
            ...
            implode(' AND ', array(
                ...
                "({$compoundTableAlias}.store_id = {$storeId})", 
            )), 
            array()
        );
    ...
    protected function _getRuleProductsStmt2($fromDate, $toDate, $productId = null, $websiteId)
    {
        ...
        $order      = array(
            'rp.website_id', 'rp.store_id', 'rp.customer_group_id', 'rp.currency', 
        ...
        $select->joinLeft(
            ...
            implode(' AND ', array(
                ...
                "({$globalCompoundTableAlias}.store_id = 0)", 
            )), 
            array()
        );
    ...
    protected function _calcRuleProductPrice($ruleData, $productData = null)
    {
        ...
        return $this->getAdvancedPricingHelper()->getCatalogRuleHelper()->roundPrice($productPrice);
    }
    /**
    ...
    protected function _saveRuleProductPrices($arrData)
    {
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            $header = 'replace into '.$this->getTable('catalogrule/rule_product_price').' (
                    rule_date, website_id, store_id, customer_group_id, currency, product_id, rule_price, latest_start_date, earliest_end_date
                ) values ';
            $rows = array();
            $productIds = array();
     ...
     * @return float|bool
     */
    public function getRulePrice2($date, $wId, $sId, $gId, $curr, $pId)
    {
        $data = $this->getRulePrices2($date, $wId, $sId, $gId, $curr, array($pId));
        if (isset($data[$pId])) {
            return $data[$pId];
        }
        return false;
     ...
     * @return array
     */
    public function getRulePrices2($date, $websiteId, $storeId, $customerGroupId, $currency, $productIds)
    {
     ...
     * @return array
     */
    public function getRulesFromProduct2($date, $websiteId, $storeId, $customerGroupId, $currency, $productId)
    {
        $adapter = $this->_getReadAdapter();
        if ($this->getVersionHelper()->isGe1700()) {
            if (is_string($date)) {
    ...
    public function applyToProduct2($rule, $product, $websiteIds, $storeIds)
    {
        ...
        if (!$this->getVersionHelper()->isGe1800()) {
            ...
            if (!$this->validateProduct2($rule, $product, $websiteIds, $storeIds)) {
                $write->delete($this->getTable('catalogrule/rule_product_price'), array(
                    $write->quoteInto('product_id = ?', $productId),
                ));
                $write->commit();
