 ...
 */
class Innoexts_AdvancedPricing_Model_Catalog_Product_Attribute_Backend_Groupprice 
    extends Mage_Catalog_Model_Product_Attribute_Backend_Groupprice 
{
    ...
    {
        return $this->getAdvancedPricingHelper()->getProductHelper();
    }
    /**
    ...
    public function validate($object)
    {
        ...
        if ($productHelper->isGroupPriceFixed($object)) {
            $duplicateMessage = $helper->__('Duplicate website group price store, customer group and currency.');
        } else {
            $duplicateMessage = $helper->__('Duplicate website group price store and customer group.');
        }
        $duplicates = array();
        ...
        if (($priceHelper->isStoreScope() || $priceHelper->isWebsiteScope()) && $object->getStoreId()) {
            ...
            foreach ($origPrices as $price) {
                if ($priceHelper->isAncestorData($price, $storeId)) {
                    $compare        = $this->getDataKey($price);
                    $duplicates[$compare] = true;
                }
            }
     ...
     * @return array
     */
    public function preparePriceData2(array $priceData, $productTypeId, $websiteId, $storeId, $currency)
    {
        $helper                 = $this->getAdvancedPricingHelper();
        $priceHelper            = $this->getProductPriceHelper();
    ...
    public function afterLoad($object)
    {
        ...
        if ($priceHelper->isGlobalScope()) {
            ...
            $websiteId          = $helper->getCoreHelper()->getWebsiteIdByStoreId($store->getId());
            $storeId            = null;
        ...
        $data = $resource->loadPriceData2($object->getId(), $websiteId, $storeId, $currency);
        foreach ($data as $k => $v) {
