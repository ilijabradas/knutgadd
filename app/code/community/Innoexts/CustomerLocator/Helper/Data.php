    ...
    extends Innoexts_Core_Helper_Abstract 
{
     ...
     * @return string
     */
    public function getCustomerAddressHash()
    {
    ...
    public function applyCustomerAddressToQuoteAddress($quoteAddress, $overwrite = false)
    {
        ...
        $quote                  = ($quoteAddress) ? $quoteAddress->getQuote() : null;
        $store                  = ($quote) ? $quote->getStore() : $this->getStore();
        $config                 = $this->getConfig();
        if (
            $coreHelper->isAdmin() || 
            (!$config->isSynchronizeQuoteAddress($store)) || 
            (
        ...
        $customerAddressId      = $coreHelper
            ->getCustomerHelper()
            ->getAddressIdByAddress($customerAddress);
     ...
     * @return $this
     */
    public function applyCustomerAddress($quote = null)
    {
        if (!$quote) {
            $quote                  = $this
                ->getCoreHelper()
        ...
        foreach ($quote->getAllAddresses() as $quoteAddress) {
            $this->applyCustomerAddressToQuoteAddress($quoteAddress, true);
    ...
    {
        $coreHelper             = $this->getCoreHelper();
        $checkoutHelper         = $coreHelper->getCheckoutHelper();
        $quote                  = ($quoteAddress) ? $quoteAddress->getQuote() : null;
        $store                  = ($quote) ? $quote->getStore() : $this->getStore();
        $config                 = $this->getConfig();
        if (
            ...
            (!$quote) || 
            (!$config->isSynchronizeQuoteAddress($store)) || 
            (!$config->isAdjustAddressOnCheckout($store)) || 
            (
                $quote->isVirtual() && 
            ...
            (!in_array(
                $coreHelper->getFullControllerName(), 
                $checkoutHelper->getFullControllerNames()
            ))
        ) {
            return $this;
        }
